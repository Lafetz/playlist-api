...
export const createPlaylistHandler=[...validatePlaylistCreate,validateRequest,async (req: Request, res: Response, next: NextFunction) => {
  try {
   ...
   //@ts-ignore
    ...
    const playlist=await PlaylistService.createPlaylist(userId,{title,description})
...
export const getPlaylistsHandler = [...validateQueryPlay,validateRequest,async (req: Request, res: Response, next: NextFunction) => {
  try {
        //@ts-ignore
   const userId=req.user.id
    ...
    //@ts-ignore
    const playlists = await PlaylistService.getPlaylists(userId,{limit:limitNumber,offset:pageNumber,sort:sortOption})
...
export const updatePlaylistHandler =[ ...validateParam ,...validatePlaylistUpdate,validateRequest, async (req: Request, res: Response, next: NextFunction) => {
  try {
   ...
   const userId=req.user.id
    ...
    const updatedPlaylist = await PlaylistService.updatePlaylist(id, { title, description });
    if (!updatedPlaylist) {
      throw new NotFoundError()
    }
